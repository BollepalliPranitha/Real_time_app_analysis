...
try:
    ...
    if check_s3_path(spark, staging_path):
        try:
            ...
            live_df = spark.read.schema(live_streaming_schema).parquet(staging_path)
            ...
            logger.info(f"Read and deduplicated live_streaming: {row_count} rows")
            logger.info(f"Input schema: {live_df.schema}")
        ...
        if fact_row_count > 0:
            try:
                ...
                fact_df.write \
                    ...
                    .save()
                logger.info(f"Successfully wrote fact_live_streaming_interactions: {fact_row_count} rows")
            except Exception as write_e:
